...
const MentalHealthChatbot = () => {
  ...
  const fetchResponse = async (query) => {
    ...
    setLoading(true);
    const geminiAPIKey = import.meta.env.VITE_GEMINI_API_KEY; // Replace with actual key
  ...
  return (
    <div className="flex flex-col w-full max-w-2xl mx-auto h-[80vh] bg-green-100 shadow-lg rounded-xl p-4">
      <motion.h2 
        initial={{ opacity: 0, y: -10 }} 
        animate={{ opacity: 1, y: 0 }} 
        className="text-2xl font-semibold text-green-800 mb-4 flex items-center gap-2"
      >
        <Bot className="w-6 h-6 text-green-600" /> Mental Wellness Chatbot
      </motion.h2>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 p-2">
        {messages.map((msg, index) => (
          <motion.div
            ...
            className={`p-3 rounded-lg max-w-[80%] ${msg.sender === "user" ? "ml-auto bg-green-600 text-white" : "mr-auto bg-white text-gray-700 border"}`}
          >
            {msg.sender === "user" ? (
              <div className="flex items-center gap-2"><User2 className="w-4 h-4" /> {msg.text}</div>
            ) : (
              <div className="flex items-center gap-2"><Bot className="w-4 h-4 text-green-600" /> {msg.text}</div>
            )}
          </motion.div>
        ))}
      ...
      <div className="flex items-center p-2 bg-white rounded-lg shadow mt-2">
        ...
        <motion.button 
          whileTap={{ scale: 0.95 }}
          onClick={handleSend}
          className="ml-2 bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-1"
        >
